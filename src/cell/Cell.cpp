...
void Cell::collision(std::mt19937& gen, std::mt19937& gen01)
{   
    ...
    // }else{
        ...
        // std::cout << M_candidate<<std::endl;
        std::uniform_int_distribution<int> dis(0, N_particles - 1);
        std::uniform_real_distribution<double> dis01(0, 1);
        for(int i = 0; i < M_candidate; ++i){
            // auto index1 = static_cast<int>((N_particles - 1) * randomgenerator->getrandom01());
            // auto index2 = static_cast<int>((N_particles - 1) * randomgenerator->getrandom01());
            unsigned int index1 = dis(gen);
            unsigned int index2 = dis(gen);
            while(index1 == index2)
                index2 = dis(gen);

            // while(index1 == index2){
            //     index2 = static_cast<int>((N_particles - 1) * randomgenerator->getrandom01());
            // }
            if(index1 != index2){
                auto particle1 = m_particles[index1];
                auto particle2 = m_particles[index2];
    
                auto v_rel = particle1->getvelocity() - particle2->getvelocity();
                ...
                if(rand01 < (Src / Srcmax)){
                    ...
                    double cosr = 2.0 * dis01(gen01) - 1.0;
                    double sinr = sqrt(1.0 - cosr * cosr);
                    double phi = 2.0 * M_PI * dis01(gen01);
